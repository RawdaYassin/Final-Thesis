...
\begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{2.9cm}XXXX}
\toprule
\textbf{Property} & \makecell{\textbf{Hodgkin--}\\\textbf{Huxley}} & \textbf{Izhikevich} & \makecell{\textbf{Spike}\\\textbf{Response}} & \textbf{LIF} \\
\midrule
...
\toprule
\textbf{Property} & \makecell{\textbf{Rate}\\\textbf{Coding}} & \makecell{\textbf{Latency}\\\textbf{Coding}} & \makecell{\textbf{Delta Mod}\\\textbf{ulation}} & \makecell{\textbf{Phase}\\\textbf{Coding}} \\
\midrule
...
\begin{tikzpicture}[
    >=Stealth,
    model/.style={draw, rounded corners, minimum height=1.1cm, minimum width=2.4cm, align=center, font=\footnotesize, fill=#1!12, draw=#1!50, thick},
    model/.default=blue,
    vote/.style={draw, diamond, minimum height=1.5cm, minimum width=2.2cm, align=center, font=\footnotesize, fill=#1!15, draw=#1!50, thick, inner sep=2pt},
    vote/.default=orange,
    arr/.style={->, thick, gray!70!black},
...
\paragraph{Statistical Significance Caveat} The performance differences reported across models and variants are based on single-run evaluations with fixed random seeds. While the observed differences (e.g., SNN AUC 87.53\% vs.\ Random Forest 85.21\% on the Base variant, a gap of 2.32 percentage points) are consistent with the relative model rankings reported in prior work on the BAF dataset~\cite{jesus2022bank,uwaoma2024detecting}, formal statistical significance testing (e.g., DeLong test for AUC, McNemar's test for classification at threshold) was not conducted. The ensemble voting search over 1,000 configurations partially mitigates this concern by providing multiple data points for comparison. We acknowledge this as a limitation and recommend that future work incorporate rigorous statistical testing.
